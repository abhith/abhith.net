...
Azure is my favorite cloud provider. We kind of relying upon Azure for most of the services.
...
In my case, we have a monorepo that contains several ASP.NET Core microservices and the folder structure is inherited from [eShopOnContainers](https://github.com/dotnet-architecture/eShopOnContainers).
...
- ACR service connection in Azure DevOps (acr-name-connection is the name of the connection). You can add it under `Azure DevOps > Project > Project Settings > Service Connections`.
...
In the example below, `aks-test-name` variable group has my AKS cluster values for the TEST environment.
...
In the CD pipeline, I have defined two stages, one for TEST and one for PROD. The main difference between them is in the variable group used.
`aks-test-name` variable group has the TEST cluster values and you guessed right, `aks-prod-name` has the PROD cluster values.
And the difference between `cd-steps-template.yml` and `cd-steps-template-prod.yml` is the prod file has some additional chart value overrides with values for the PROD environment.
