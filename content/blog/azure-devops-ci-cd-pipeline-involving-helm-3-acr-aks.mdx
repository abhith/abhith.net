...
Azure is my favorite cloud provider. We kind of relying Azure for most of the services.
Our code goes to Azure DevOps, we use Azure Container Registry (ACR) to host our docker container images and our kubernetes clusters running in Azure Kubernetes Service (AKS).

We configured the the CI/CD pipelines in Azure DevOps.
In my case we have a monorepo which contains several ASP.NET Core microservices and the folder structure is inherited from [eShopOnContainers](https://github.com/dotnet-architecture/eShopOnContainers).
The folder structure look like below,
...
One of the great article that helped me to initially setup the CI/CD is given below.
Its kind of outdated now since it is using `az acr helm` commands which were deprecated later.
But it is still worth reading. So defenetily check it out.
...
The Helm chart template for each project is located under `deploy > k8s > helm`

### Prerequisites

- ACR service connection from Azure DevOps (acr-name-connection is the name of the connection). You can add it under `Azure DevOps > Project > Project Settings > Service Connections`.
- A variable group `acr-name-variable-group` which contains the ACR related variables (registryName, registryLogin and registryPassword).
...
- `build.sourceBranchName` as the image tag and chart version is handy if you are following **Gitflow** or similar branching convention, so each release (eg, refs/tags/project-one/2.2.6) will generate Docker image and Helm chart with the same version.
...
4. Copy some additional files. Which we can use to override Helm chart values.
...
variables:
  - group: acr-name-variable-group
  - template: ../common/ci-vars-template.yml
    parameters:
      projectName: "project-one"
      dockerRegistryServiceConnectionName: "acr-name-connection"
...
Once the CI completed successfully, we can setup CD. Which will install the Helm chart to AKS.

Before we do this, we need

- Service principal credentials (aksSpId, aksSpSecret)
...
You can get the `aks-resource-id` by,

```shell
az aks show -n aks-name -g aks-resource-group-name --query id -o tsv
```

The above command will output `aksSpSecret`.
...
- Need to attach ACR with AKS

```bash
az aks update -g aks-resource-group-name -n aks-name --attach-acr acr-resource-id
```

To get acr-resource-id,

```bash
az acr show -n acr-name -g acr-resource-group-name --query id -o tsv
```

- Another varible group(s) which has following AKS details
  - aks - AKS name
  - rg - AKS resource group
  - aksSpTenantId - Tenant Id
  - aksSpId and aksSpSecret .

In the example below, `aks-test-name` varibale group has my AKS cluster values for TEST environment.
...
- Authenticate ACR with the ACR credentials (The same credentials we used in CI pipeline)
...
In the CD pipeline, I have defined two stage, one for TEST and one for PROD. The main difference between them is in the variable group used.
`aks-test-name` varibale group have the TEST cluster values and you guessed right, `aks-prod-name` has the PROD cluster values.
And difference between `cd-steps-template.yml` and `cd-steps-template-prod.yml` is the prod file has some additional chart value overrides with values for PROD environment.
...
- CD pipeline is also YAML based (You gonna like it), hence create it like a regular pipeline (Not RELEASE) in the Azure DevOps, and choose the `cd-pipeline.yml` after choosing to create pipeline based on "Existing Azure Pipelines YAML file".
...
If you have any grey area in this article, feel free to ask me in the comments below :point_down:.

Suggestions for improvements are always welcome, send me a PR, this article itself is opensource :heart:
