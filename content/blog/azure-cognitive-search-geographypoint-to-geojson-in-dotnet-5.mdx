...
We need to have the nuget package `Microsoft.Spatial` in order to use the `GeographyPoint` type.
...
But when you try to index a document with the same type by,
