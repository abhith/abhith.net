...
In a **microservice** world, the machine (microservice A) to machine (microservice B) communications can be secured using an an **OAuth 2.0** compatible token service, **IdentityServer** in our case. i.e In order to successfully call microservice B, the microservice A need to get an access token first via client credentials grant type from the IdentityServer with the right scope for the operation.

- microservice A calls IdentityServer to get an `access token`.
- microservice A then calls microservice B with access token in the authorization header (bearer token).
- microservice B validates the access token, responds back to microservice A with valid response.

The above steps are fine for a single call, but in case of multiple calls from A to B, A doesn't need to get IdentityServer token everytime since each token has its own validity, so bacically A can reuse the token until it expires.

For the token reusing, you don't need to implement the caching and refreshing token logic anywhere since some good people who are maintaining [IdentityModel.AspNetCore](https://github.com/IdentityModel/IdentityModel.AspNetCore) already done that. All we need to do is setup it, and consume.
...
- microservice B has endpoints,
  ...
  - and to fetch the orders `GET /orders` endpoint, scope for that one is `orders.read`.
...
```cs
public class CampaignAppConsts
{
...
Configuration done, now lets consume the `HttpClient`.
Here I'm using `Flurl` to make the HTTP request, if you prefer direct HttpClient usage, you can do that way, basically you don't need to do anything special to get the access token since it is already taken care of.
...
That's it. You can use the same way to call the create order endpoint since the client has access token with both scopes.
